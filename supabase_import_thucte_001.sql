...
(gen_random_uuid()::text,
  'opening', 'Bot tự động chào khách đầu tiên',
  E'Chào em nha! Em đang muốn tham khảo "Trọn gói chụp ảnh cưới" hay "Váy cưới"? Để chị tư vấn chi tiết cho em nhé!',
  ARRAY['bot', 'chào', 'tự động', 'đầu tiên'], true, 100
),

(gen_random_uuid()::text,
  'opening', 'Sale tiếp nhận sau bot',
  E'Chào em nha. Em muốn tư vấn về chụp ảnh cưới hay váy cưới để chị tư vấn cho em nè',
  ARRAY['chào', 'tiếp nhận', 'mở đầu'], true, 101
),

(gen_random_uuid()::text,
  'discovery', 'Khai thác nhu cầu — studio hay ngoại cảnh',
  E'Vợ chồng em có ngày cưới chưa bé, vợ chồng em dự định chụp studio hay ngoại cảnh nè em',
  ARRAY['khai thác', 'nhu cầu', 'ngày cưới', 'studio', 'ngoại cảnh'], true, 102
),

(gen_random_uuid()::text,
  'discovery', 'Tạo urgency khi khách gần ngày cưới',
  E'Vậy thời gian này vợ chồng em sắp xếp chụp là vừa thời gian chọn ảnh và duyệt ảnh nữa em nè',
  ARRAY['urgency', 'gần ngày', 'thời gian', 'chọn ảnh'], true, 103
),

(gen_random_uuid()::text,
  'value_prop', 'Giải thích thời gian — makeup 2 tiếng không di chuyển',
  ...
  ARRAY['thời gian', 'makeup', '2 tiếng', 'không di chuyển', 'concept'], true, 104
),

(gen_random_uuid()::text,
  'value_prop', 'Tự chọn makeup tóc concept trang phục hoàn toàn',
  ...
  ARRAY['makeup', 'tóc', 'concept', 'tự chọn', 'trang phục', 'sở thích'], true, 105
),

(gen_random_uuid()::text,
  'offer', 'Gửi bảng giá kèm USP H2O Studio',
  ...
  ARRAY['bảng giá', 'USP', 'giá trị', 'khác biệt', 'offer', 'gói chụp'], true, 106
),

(gen_random_uuid()::text,
  'fomo', 'Ưu đãi 48h — quà tặng 3.500.000đ',
  ...
  ARRAY['ưu đãi', '48h', 'fomo', 'quà tặng', 'book online', '3.5 triệu', 'voucher'], true, 107
),

(gen_random_uuid()::text,
  'fomo', 'Gửi thêm concept để khách dễ quyết định',
  ...
  ARRAY['concept', 'gửi hình', 'quyết định', 'tham khảo', 'do dự'], true, 108
),

(gen_random_uuid()::text,
  'closing', 'Lấy thông tin sau khi khách chốt gói',
  E'Vck em đăng ký giữ lịch chụp gửi chị xin thông tin tên và số điện thoại của vợ chồng em nhé',
  ARRAY['chốt', 'thông tin', 'tên', 'sđt', 'giữ lịch'], true, 109
),

(gen_random_uuid()::text,
  'closing', 'Gửi thông tin chuyển khoản đặt cọc',
  E'Em chuyển khoản ghi họ tên của em và gửi chị ảnh chụp màn hình là được nhớ\n\n➡️ MB Bank – STK: 9098688688888 – NGUYEN THU THUY\n➡️ Vietcombank – STK: 0031000367971 – NGUYEN THU THUY',
  ARRAY['chuyển khoản', 'cọc', 'mb bank', 'vietcombank', 'đặt cọc', '1 triệu'], true, 110
),

(gen_random_uuid()::text,
  'closing', 'Xác nhận sau nhận cọc + hẹn lịch chụp',
  E'Chị nhận nha bé\n\nChị hẹn vck em 8h30 [ngày chụp] bé nha\n\nChị gửi phần dặn dò ngày chụp vợ chồng em nhé\n\n[GỬI CHECKLIST CHUẨN BỊ]',
  ARRAY['xác nhận', 'nhận cọc', 'hẹn lịch', 'checklist', '8h30'], true, 111
),

(gen_random_uuid()::text,
  'pre_shoot', 'Dặn dò chuẩn bị ngày chụp',
  ...
  ARRAY['chuẩn bị', 'checklist', 'ngày chụp', 'dặn dò', 'pre_shoot', '8h30'], true, 112
),

(gen_random_uuid()::text,
  'followup', 'Hỏi thăm feedback sau buổi chụp',
  ...
  ARRAY['feedback', 'sau chụp', 'hỏi thăm', 'giới thiệu', 'followup', 'giao ảnh'], true, 113
);
